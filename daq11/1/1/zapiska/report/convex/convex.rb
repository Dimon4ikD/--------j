...
class Figure
  ...
  def get_sum
    ...
    pts.size.times do |i|
      ...
      if m
        #выбераем точку на ребре не совпадающей с точкой пересечения
        cp = (m == p1) ? p2 : p1
        #выбераем точку на прямой не совпадающей с точкой пересечения
        cl = (m == $points[0]) ? $points[1] : $points[0]
        #вычисляем угл между ребром и премой и добавляем его в сумму
