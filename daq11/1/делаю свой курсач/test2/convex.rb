...
################################################начиная с полигона все переделать!
...
=begin
  ...
  def add(t)
    ...
    unless i.nil?
      ...
      if i == -1
        # "прокручиваем" массив @points, чтобы ребро
        # @points[-2],@points[-1] не было освещённым
        while t.light?(@points[-2],@points[-1])
          @points.unshift(@points.delete_at(-1))
        end
      end

      # находим "последнее" освещённое ребро
      j = i + 1
      while j < @points.size-1 && t.light?(@points[j],@points[j+1])
        j += 1
      end
  ...
  private
  # Нахождение какого-нибудь освещённого ребра
  def light_edge(t)
    for p in -1..(@points.size-2)
      return p if t.light?(@points[p],@points[p+1]) 
    end
    nil
  end
=end
